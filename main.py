...
def main():
    ...
    parser.add_argument("-p", "--password", help="the password to use for authentication (must be exactly 8 characters and be the same on client and server); if unspecified, a default is used")
    parser.add_argument("-k", "--key", help="the AES key to use for encryption (must be 32 characters and the same on client and server); if unspecified, a default is used")
    ...
    if args.password and len(args.password) != 8:
        print("-p/--password: password length must be exactly 8 characters.")
        sys.exit(1)
    
    if args.password and len(args.password) != 32:
        print("-k/--key: key length must be exactly 32 characters.")
        sys.exit(1)
